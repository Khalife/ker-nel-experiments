...
#ok = returnNewScores(all_mentions_id.index('EL013766'), 0.5)
#pdb.set_trace()
#
#results = []
#become_mis_cities_id = ['EL013053', 'EL013056', 'EL013052', 'EL013062', 'EL013046', 'EL013059', 'EL013049', 'EL013061', 'EL013047']
#new_become_mis_cities_id = ['EL013003', 'EL013002', 'EL013001', 'EL013009', 'EL012993', 'EL012996', 'EL012997', u'EL012994', u'EL013059']
#new_bis_become_mis_cities_id = ['EL013061', 'EL013062', 'EL013052', 'EL013053', 'EL013049', 'EL013046', 'EL013059', 'EL013056', 'EL013047']
#for mi in new_bis_become_mis_cities_id:
#    result = returnNewScores(all_mentions_id.index(mi), 0.5)
#    results.append(result)
#
##pdb.set_trace()
#hardResults = [int(res[0] == np.argmin([re[2] for re in res[1]])) for res in results] 
#softResults = [int(res[1][0][0] < res[1][min([res[0], 6])][0]) for res in results]
#
#results2 = []
#for mi in mentions_cities_id:
#    results2.append(returnNewScores(all_mentions_id.index(mi), 0.5))
#
#hardResults2 = [int(res[0] == np.argmin([re[2] for re in res[1]])) for res in results2]
#softResults2 = [int(res[1][0][0] < res[1][min([res[0], 6])][0]) for res in results2]
#
#pdb.set_trace()
##
##ok0 = returnNewScores(all_mentions_id.index('EL002719'), 0.3)
##for alpha_ in [0.3, 0.4, 0.5, 0.75, 0.95]:#[0., 0.1, 0.2, 0.4, 0.5, 0.75, 0.90, 0.95, 1]:
##    print(alpha_)
##    final_alpha_ = finalReturn(alpha_, ["City"])
##    final_alphas.append(final_alpha_[0])
##    final_mis.append(final_alpha_[1])
##    final_become_mis.append(final_alpha_[2])
##    final_become_good.append(final_alpha_[3])
##    print("Final " + str(len(final_alpha_[0]))) 
#
#
#pdb.set_trace()
...
def subgraph_adhoc(graph_, nodes):
    new_subgraph = {}
    for node in nodes:
        new_subgraph[node] = set([])
        neighbors_node = graph_[node]
        for neighbors_node in neighbors_node:
            new_subgraph[node].add(neighbors_node)
...
def exploreResult(object_list_):
    
    #### Exploration ####
    start = time.time()
    #print("Exploring graph...")
    gold_entity_id = object_list_["gold_entity_id"]
    gold_index = entityIdToIndex[gold_entity_id]
    nb_seeds = 10
    nb_levels = 3 
    categories = ["ORG", "GPE", "PER", "UKN"]
    sorted_rank = object_list_["sorted_RANKS"]
    mention_id = object_list_["mention_id"]
    #seed_indexes = [i for i in range(5)] + [10, 20, 30, 50, 100]
    seeds_categories = {}
    for category in categories: 
        seeds_categories[category] = [entityIdToIndex[sorted_rank[category][i]["entity_id"]] for i in seed_indexes]

    seeds = sum([seeds_categories[cat] for cat in categories], [])   
  
    explored_nodes = []
    explored_NODES = []
    explored_NODES_return = {}
    for cat in categories:
        for i in range(len(seeds_categories[cat])):
            #if graph_degrees[seeds_categories[cat][i]] < 5000:
            if G.degree(seeds_categories[cat][i]) < 5000:
                #print("seed " + str(i))
                explored_nodes = bfs_edges_level_myGraph(G, nb_levels, seeds_categories[cat][i])
                explored_NODES_return[cat] = [e_n[1] for e_n in explored_nodes if e_n[1] not in seeds]


    #explored_NODES_return
    #explored_NODES = sum([explored_NODES_return[cat] for cat in categories],[])

    end = time.time()
    #print(str(end-start) + " s to explore nodes") 
    nodeFrequency = {}
    key_nodes = sum([explored_NODES_return[cat] for cat in categories], [])
    key_nodes_return = key_nodes
    key_nodes = [kn for kn in key_nodes if entityIdToType[entityIndexToId[kn]] in categories]
        
    for kn in set(key_nodes):
        nodeFrequency[kn] = 0
    ...
    dic_return_entities = {}
    for cat in categories:
        dic_return_entities[cat] = []
        sorted_rank_cat = {} 
        for entity_dic in sorted_rank[cat]:
            sorted_rank_cat[entity_dic["entity_id"]] = entity_dic["score"]    

        cat_id_frequencies = [entityIndexToId[nf] for nf in nodeFrequency.keys() if entityIndexToType[nf] == cat]
        cat_frequencies = [nodeFrequency[nf] for nf in nodeFrequency.keys() if entityIndexToType[nf] == cat]
        try:
            max_cat_frequency = max(cat_frequencies)
        except:
            continue     


        cat_normalized_frequencies = [cf/float(max_cat_frequency) for cf in cat_frequencies]
        ...
        entities_index_to_keep = []
        cf_index = -1
        for cf in cat_id_frequencies:
            cf_index += 1
            try:
                cat_loc_entities[cf] = sorted_rank_cat[cf]
                entities_index_to_keep.append(cf_index)
            except:
                continue

        if len(entities_index_to_keep) < len(cat_id_frequencies):
            cat_normalized_frequencies = [cat_normalized_frequencies[i] for i in entities_index_to_keep]
            cat_id_frequencies = [cat_id_frequencies[i] for i in entities_index_to_keep] 

        
        cat_alpha_scores = {}
        for cnf, cf in zip(cat_normalized_frequencies, cat_id_frequencies):
            cat_alpha_scores[cf] = alpha*cnf + (1-alpha)*sorted_rank_cat[cf]
        cat_sorted_alpha_entities = sorted(cat_alpha_scores.items(), key=operator.itemgetter(1), reverse=True)

        #cat_sorted_final_entities = [entityIndexToId[seed] for seed in seeds_categories[cat]] + [sr[0] for sr in cat_sorted_alpha_entities][:nb_seeds] 
        cat_sorted_final_entities = [entityIndexToId[seed] for seed in seeds_categories[cat]] + [sr[0] for sr in cat_sorted_alpha_entities][:150]
        print(len(cat_sorted_final_entities))
        if cat == CATEGORY: 
            try:
                gold_local_rank = cat_sorted_final_entities.index(gold_entity_id)
            except:
                gold_local_rank = -1
                

            if ( gold_local_rank > -1 ): #and ( gold_local_rank < 20 ) :
                for i in range(min([150,len(cat_sorted_final_entities)])):
                    if i != gold_local_rank:
                        dic_return_entities[cat].append(cat_sorted_final_entities[i])
    ...
    dic_return = {}
    try:
        dic_return["gold_rank"] = gold_local_rank
    except:
        return -1, [-1 for i in range(19)]
    dic_return["mention_id"] = mention_id
    #dic_return["gold_entity"] = {gold_entity_id: entityIdToText[gold_entity_id]}
    dic_return["gold_entity"] = gold_entity_id
    dic_return["corrupted_entities"] = dic_return_entities

    dic_return_seeds = {}
    for cat in categories:
        dic_return_seeds[cat] = seeds_categories[cat]

    dic_return["other_seeds"] = dic_return_seeds
    #dic_return["new_explored_nodes"] = explored_NODES_return 
    
    ##########################################################################################
    ###########################################################################################################     
    
    if gold_local_rank < 0:
        return -1
    else:

        corrupted_indexes = {}
        for cat in ["PER", "ORG", "GPE", "UKN"]:
            corrupted_ids = [key for key in dic_return["corrupted_entities"][cat]]
            corrupted_indexes[cat] = [entityIdToIndex[id_] for id_ in corrupted_ids]
    
    
        #seeds = seeds_categories
        #seeds_except_gold_CAT = [ei for ei in seeds["GPE"] if ei != gold_index]
        #seeds_NO_CAT = sum([[ei for ei in seeds[cat]] for cat in ["PER", "ORG", "UKN"]], [])
                                                                                                                                                                         
                                                                                                                                                                         
        corrupted_indexes_total = sum([corrupted_indexes[cat] for cat in ["PER", "ORG", "GPE", "UKN"]],[])
                                                                                                                                                                         
        exp_CAT_nodes = corrupted_indexes["GPE"][:min([len(corrupted_indexes["GPE"]),max([19, gold_local_rank])])]
                                                                                                                                                                         
        test_neighbors_GOLD_CAT = [n for n in G.neighbors(gold_index) if n not in corrupted_indexes_total]
        test_neighbors_NO_GOLD_CAT = sum([[n for n in G.neighbors(segc) if n not in corrupted_indexes_total] for segc in exp_CAT_nodes], [])
                                                                                                                                                                         
        test_neighbors_CAT = list(set(test_neighbors_GOLD_CAT + test_neighbors_NO_GOLD_CAT))
        #for ei in seeds_neighbors_CAT:
        #    val_map[ei] = "0.75" # ORANGE
                                                                                                                                                                         
                                                                                                                                                                         
                                                                                                                                                                         
        #H = G.subgraph([gold_index] + corrupted_indexes_total + seeds_neighbors_CAT)
                                                                                                                                                                         
        #H_exp = G.subgraph([gold_index] + exp_CAT_nodes)
        degrees_exp = []
        first_neighbors = G.graph[gold_index]
        first_neighbors = [fn for fn in first_neighbors if fn not in corrupted_indexes_total]         
        degrees_exp.append(len(first_neighbors))
        for snc in exp_CAT_nodes:
            first_neighbors = G.graph[snc]
            first_neighbors = [fn for fn in first_neighbors if fn not in corrupted_indexes_total]
            degrees_exp.append(len(first_neighbors))
        ...
        #pdb.set_trace()                                                                                                                                                                        
        #if gold_local_rank < 20:
        #    print(degrees_exp)
        return gold_local_rank, degrees_exp
...
#CPU
#results = []
#object_list = generateDataCpu(ranks_filename)
#index_mentions = 1
#for obj_list in object_list:
#    print("mention "  + str(index_mentions))
#    results.append(exploreResult(obj_list))
#    index_mentions += 1  
#
##pdb.set_trace()
#writeFileNameSplit = ranks_filename.split("-")
#writeFileName = writeFileNameSplit[0] + "-NEW-ANALYSIS-mentions-" + writeFileNameSplit[1]
#fileResults = open(writeFileName, "w")
#for res in results:
#    fileResults.write("%s\n" %  res)

#SPARK
#data_file = sc.textFile(ranks_filename)
#object_list_spark = data_file.map(lambda line: generateData(line))
###object_list_spark = sc.parallelize(object_list, 100)
#object_list_results =  object_list_spark.map(lambda line: exploreResult(line))
#### 
#writeFileNameSplit = ranks_filename.split("OWT-Scores")
#print(writeFileNameSplit)
#writeFileName = writeFileNameSplit[0] + "-NEW-ANALYSIS-NEL-" + writeFileNameSplit[1]
#object_list_results.saveAsTextFile(writeFileName)
